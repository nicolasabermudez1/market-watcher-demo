...
def _generate_weekly_digest(category: str, risks: list, news_items: list, run_date: str = "") -> str:
    ...
    title_run = title.add_run("Category Watcher — Weekly Intelligence Digest")
    ...
    sub_run = sub.add_run(f"{category}  ·  {date_str}  ·  Prepared by Category Watcher Agent")
    ...
    summary_text = (
        ...
        f"The Category Watcher agent identified {len(high_risks)} HIGH and {len(med_risks)} MEDIUM priority risk signals "
    ...
    footer_run = footer_p.add_run(
        "CONFIDENTIAL — Centrica Procurement  ·  Generated by Category Watcher Agent  ·  For internal use only"
    )
...
[Este correo ha sido generado por el agente Category Watcher — sólo para revisión interna]
"""
    else:
...
[This email was drafted by the Category Watcher Agent — for internal review before sending]
"""
    return body
